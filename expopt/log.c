...
void OptimizationLog(int opt_idx) {
   char fname[4096];
   const char *logdir = getenv("EXPOPT_LOGDIR");
   int l = SYS.active;
   if(logdir != NULL)
      snprintf(fname, sizeof(fname), "%s/opt_%03i.log", logdir, opt_idx);
   else
   ...
   fprintf(SYS.logfile,
      "Opt[%i] shell %i(%s) Z=%i n=%i dirs=%i  gen=%s  method=%s%s  cores=%i\n",
